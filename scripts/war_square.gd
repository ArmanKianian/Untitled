...
var slot
func _ready() -> void:
	spawn(player_square, PLAYER_STAND, player_first_slot_position, -1)
	spawn(enemy_square, ENEMY_STAND, enemy_first_slot_position, 1)

func spawn(square, stand, first_slot_position, direction):
